...
public class DemoController {
    ...
    // @PostAuthorize
    @GetMapping("/demo5/{smth}")
    @PostAuthorize("returnObject == 'Demo 4'")  // is mainly used when we want to restrict the access to some returned value
    public String demo5() {
    ...
    // @PostFilter == the returned type must be either a Collection or an array -- doesn't work on immutable collection

    @GetMapping("/demo7")
    @PostFilter("filterObject.contains('a')")
    public List<String> demo7() {
        return List.of("abcd", "wert", "qaaz", "wrty");  // List.of creates an immutable collection so doesn't work
    }
}
